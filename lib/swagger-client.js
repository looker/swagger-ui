...
var isBinaryContentType = function(content_type) {
  return content_type && (content_type.toLowerCase().indexOf('image/') == 0);
}

var parseResponseHeaders = function (allHeaders) {
  var headers = {},
    headerArray = allHeaders.split("\n");
...
var getResponseHeader = function(xhr, name) {
  return xhr.getResponseHeader(name) || xhr.getResponseHeader(name.toLowerCase()) || null;
}

var parseIfJson = function(xhr, out) {
  var contentType = getResponseHeader(xhr, 'Content-Type');

  if (contentType) {
    if (contentType.indexOf("application/json") == 0 || contentType.indexOf("+json") > 0) {
      if (xhr.responseText && xhr.responseText !== "")
        out.obj = JSON.parse(xhr.responseText);
      else
        out.obj = {}
    }
  }
...
var processResponse = function(method, url, xhr, cb) {
  ...
  if(xhr.status >= 200 && xhr.status < 300)
    cb.response(out);
  else if(xhr.status === 0 || (xhr.status >= 400 && xhr.status < 599))
    cb.error(out);
  else
    return cb.response(out);
};

var XHRHttpClient = function (options) {
...
XHRHttpClient.prototype.execute = function (obj) {
  ...
  xhr.onreadystatechange = function () {
    switch (xhr.readyState) {
      case 2: // MSIE constant names are different
      case XMLHttpRequest.HEADERS_RECEIVED:
        if (xhr.status == 200 && isBinaryContentType(getResponseHeader(xhr, 'Content-Type'))) {
      ...
      case XMLHttpRequest.DONE:
        processResponse(method, url, xhr, cb);
        break;
    }
  };
...
JQueryHttpClient.prototype.execute = function(obj) {
  ...
  obj.complete = function(response, textStatus, opts) {
    return processResponse(obj.method, obj.url, response, cb);
  };
